/* Upgraded Pong with real-time SSE commentary integration.
   The client opens an SSE (/sse) to receive live commentary.
   It posts game state to /send-state periodically. Server proxies to OpenAI streaming
   and broadcasts commentary back via SSE. */
...
(() => {
  ...
  window.addEventListener('keydown', (e) => {
    if (e.code === 'ArrowUp' || e.code === 'ArrowDown') { keys[e.code] = true; e.preventDefault(); }
  });
  window.addEventListener('keyup', (e) => {
    if (e.code === 'ArrowUp' || e.code === 'ArrowDown') { keys[e.code] = false; e.preventDefault(); }
  });
  canvas.addEventListener('click', () => {
    if (!running) { running = true; serve(); lastTime = performance.now(); requestAnimationFrame(loop); }
    else running = false;
  });

  function serve() {
    if (playerScore === 0 && aiScore === 0) resetBall(Math.random() > 0.5 ? 1 : -1);
    else resetBall(playerScore > aiScore ? 1 : -1);
  }
  ...
  function update(dt) {
    ...
    // AI: can be adjusted by server via `aiSpeedOverride` variable
  ...
  function draw() {
    ...
    // Scores
    ctx.fillStyle = '#cfead1'; ctx.font = '28px system-ui'; ctx.textAlign='center';
    ctx.fillText(playerScore, W/2 - 60, 40); ctx.fillText(aiScore, W/2 + 60, 40);
    if (!running) {
      ...
      ctx.fillText('Click to Play / Resume', W/2, H/2-8);
    }
  }
  ...
  // Real-time integration (SSE + send-state)
  // Open SSE to receive commentary
  const sseUrl = '/sse';
  const evtSrc = new EventSource(sseUrl);
  evtSrc.onmessage = (ev) => {
    // Each message is incremental text chunk or final decision
    appendCommentary(ev.data);
    // Try parse special JSON control blocks if present
    // Model may send JSON in code fences like: {"type":"aiAdjust","aiSpeed":5}
    try {
      const parsed = JSON.parse(ev.data);
      if (parsed && parsed.type === 'aiAdjust' && typeof parsed.aiSpeed === 'number') {
        rightPaddle.speed = parsed.aiSpeed;
        appendCommentary(`(AI speed set to ${parsed.aiSpeed.toFixed(2)})`);
      }
    } catch (e) {
      // not JSON -> treat as commentary text
    }
  };
  evtSrc.onerror = (e) => {
    appendCommentary('[Commentary disconnected]');
    evtSrc.close();
  };

  function appendCommentary(text) {
    if (!text) return;
    // Simple append with timestamp
    const t = new Date().toLocaleTimeString();
    const node = document.createElement('div');
    node.innerHTML = `<small style="opacity:0.6">${t}</small> ${escapeHtml(text)}`;
    commentaryEl.appendChild(node);
    commentaryEl.scrollTop = commentaryEl.scrollHeight;
    // Short phrases to TTS
    speak(text);
  }
  function escapeHtml(s){ return s.replace(/[&<>"]/g, (c)=> ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); }

  // Periodically send state to server
  const stateIntervalMs = 400; // 400ms between updates (adjust as needed)
  setInterval(() => {
    const state = {
    ...
    // Send but don't await — best to avoid blocking the game loop
    navigator.sendBeacon ? navigator.sendBeacon('/send-state', JSON.stringify(state)) : fetch('/send-state', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(state) }).catch(()=>{});
  }, stateIntervalMs);
